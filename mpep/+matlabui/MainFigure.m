...
%             set(self.animalEdit, 'ButtonDownFcn', @(src,evt) self.animalEditCB());
        ...
        function selectAnimal(self, a)
            ...
            set(self.seriesEdit, 'String', num2str(self.series));
        ...
        function seriesEditCB(self)
            num = str2double(get(self.seriesEdit, 'String'));
            % Check valid number. If not revert input.
            if isnan(num) || ~isscalar(num) || num < 1
                set(self.seriesEdit, 'String', self.series);
                return;
            end
            changed = self.series ~= num;
            self.series = num;
        ...
        function runButtonCB(self)            
            ...
            preMake = false;
            try
                exp = mpepcore.Protocol(self.animal, self.series, rp, reps, preMake, comment);
            catch e
                msgbox(e.message, 'Error');
                return;
            end
            ...
            Logger.singleton.complete(self.series, num, comment);
        end
